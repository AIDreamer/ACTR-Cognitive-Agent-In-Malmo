...
proc select_graphic_trace {key current} {
  ...
  button $win.stop_gt -command [list draw_horiz_items $win $key $current] -text "Get trace" -font button_font

  button $win.redisplay -command [list display_horiz_data $win 1 $key $current] -text "Redisplay" -font button_font
  ...
  place $win.canvas1  -x 0 -y 0 -width 150 -relheight 1.0 -height -80
...
add_history_button select_graphic_trace "Graphic Trace (h.)" :save-buffer-trace "Graphic trace" left get-environment-buffer-trace default-save-history-info
...
proc draw_horiz_items {win key current} { 
  ...
  if $current {
    send_environment_cmd "create list-handler $win.stop_gt $win.return (lambda (x) (declare (ignore x)) (hor-graphic-trace-return '$key)) () $key"
  } else {
    send_environment_cmd "create list-handler $win.stop_gt $win.return (lambda (x) (declare (ignore x)) (hor-graphic-trace-return '$key)) ()"
  }
  ...
  display_horiz_data $win 1 $key $current
...
proc display_horiz_data {win level key current} {
    ...
    foreach x $result {
      switch [lindex $x 0] {
        ...
        size { 
          ...
          while {$x <= $size} {
            $win.frame.canvas create line $x 0 $x 400 -width 1 -f gray -tag [list grid trace_items]
            $win.frame.canvas create text $x 400 -text [format "%.3f" [expr 0.001 * $x]] -anchor sw -font graphic_trace_font -tag [list grid trace_items]
            incr x 50
          }
        }
...
#### Just pass it off to the central history code and let it figure it out
#### give it the buffer name and the start time along with key and current

          $win.frame.canvas bind $box_name <ButtonPress> [list history_custom_buffer_view $win $key $current [lindex $x 10] [lindex $x 1] [lindex $x 3]]

          if { $show_text != ""} {
            $win.frame.canvas create text [expr 2 + [lindex $x 1]] [lindex $x 2] -text $show_text -anchor nw -font graphic_trace_font -tag [list trace_items trace_text $box_name] -width [expr [lindex $x 3] - [lindex $x 1]]
          }
        }
      }
    }
...
# user restricted range
   ...
   # adjust for input in seconds...
    ...
    while {$x <= $max_x} {
      $win.frame.canvas create line [expr $x - $min_x] 0 [expr $x - $min_x] 400 -width 1 -f gray -tag [list grid trace_items]
      $win.frame.canvas create text [expr $x - $min_x] 400 -text [format "%.3f" [expr 0.001 * $x]] -anchor sw -font graphic_trace_font -tag [list grid trace_items]
      incr x 50
    }
      ...
      switch [lindex $x 0] {
        ...
        rectangle {
          ...
          if {$x1 < $max_x && $x2 > $min_x} {
            ...
            if { $show_text != ""} {
              $win.frame.canvas create text [expr 2 + $x1] [lindex $x 2] -text $show_text -anchor nw -font graphic_trace_font -tag [list trace_items trace_text $box_name] -width [expr [lindex $x 3] - [lindex $x 1]]
            }
          }
        }
      }
...
# Need to bump the default scale up 1, but still want data to come over at 1pix/ms
  horiz_zoom_in $win
}
