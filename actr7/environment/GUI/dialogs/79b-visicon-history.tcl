...
proc make_visicon_history_viewer {key current} {
  
  set win [toplevel [new_variable_name .visicon_history]]
  
  wm withdraw $win

  wm geometry $win [get_configuration .visicon_history $win]
  ...
  set l3 [label $win.l3 -text "Visicon" -justify left -font label_font]

  # frame for the visicon display
  ...
  # make the selection list call the display updater for the visicon

  bind $list_box_1 <<ListboxSelect>> [list select_visicon_history %W $text_box_1 $key]


  button $win.get -text "Get History" -font button_font -command [list get_visicon_history_data $list_box_1 $key]
...
proc get_visicon_history_data {timelst key} {

  send_environment_cmd "update [get_handler_name $timelst] \
    (lambda (x) \
      (declare (ignore x)) \
      (visicon-history-time-list '$key))"
}



proc select_visicon_history {timewin target_win key} {
...
add_history_button make_visicon_history_viewer "Visicon" :save-visicon-history "Visicon history" right get-visicon-history default-save-history-info
