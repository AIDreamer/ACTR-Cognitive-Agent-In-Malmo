...
def stuff_in_front(deg, block_data):
    if block_data == None:
        return False
    elif deg == 0: # Eastward
        return not block_data[7] == u"air"
    elif deg == 180: # Westward
        return not block_data[1] == u"air"
    elif deg == 270: # Northward
        return not block_data[5] == u"air"
    elif deg == 90: # Southward
        return not block_data[3] == u"air"
## -----------------------
...
# WATER FLOW MAP (RANDOM WATER)
WATER_MAP = create_map(LENGTH, WIDTH)
for i in range(LENGTH):
    for j in range(WIDTH):
        if i % 2 == 0 and j % 2 == 1:
            WATER_MAP[i][j] = random.choice([0,1,3])
        elif i % 2 == 1 and j % 2 == 0:
            WATER_MAP[i][j] = random.choice([0,2,4])
...
def create_mission():
    ...
    for i in range(LENGTH):
        mission.drawCuboid( (CORRIDOR_LENGTH + SQUARE_WIDTH) * i, HEIGHT, 0, (CORRIDOR_LENGTH + SQUARE_WIDTH) * i + SQUARE_WIDTH - 1, HEIGHT, MISSION_WIDTH - 1, "stone")

    for j in range(WIDTH):    
        mission.drawCuboid( 0, HEIGHT, (CORRIDOR_LENGTH + SQUARE_WIDTH) * j, MISSION_LENGTH - 1, HEIGHT, (CORRIDOR_LENGTH + SQUARE_WIDTH) * j + SQUARE_WIDTH - 1, "stone")
    ...
    for i in range(2 * LENGTH - 1):
        for j in range(2 * WIDTH - 1):
            if WATER_MAP[i][j] == 1:
                mission.drawLine(SQUARE_WIDTH + COMBINED_LENGTH * (i // 2), HEIGHT + 1, COMBINED_LENGTH * (j // 2), SQUARE_WIDTH + COMBINED_LENGTH * (i//2), HEIGHT + 1, COMBINED_LENGTH * (j // 2) + SQUARE_WIDTH - 1,"water")
            elif WATER_MAP[i][j] == 3:
                mission.drawLine(COMBINED_LENGTH * ((i // 2) + 1) - 1, HEIGHT + 1, COMBINED_LENGTH * (j // 2), COMBINED_LENGTH * ((i // 2) + 1) - 1, HEIGHT + 1, COMBINED_LENGTH * (j // 2) + SQUARE_WIDTH - 1,"water")
            elif WATER_MAP[i][j] == 2:
                mission.drawLine(COMBINED_LENGTH * (i // 2), HEIGHT + 1, COMBINED_LENGTH * (j + 1) - 1, COMBINED_LENGTH * (i // 2) + SQUARE_WIDTH - 1, HEIGHT + 1, COMBINED_LENGTH * (j + 1) - 1,"water")
            elif WATER_MAP[i][j] == 4:
                mission.drawLine(COMBINED_LENGTH * (i // 2), HEIGHT + 1, SQUARE_WIDTH + COMBINED_LENGTH * (j // 2), COMBINED_LENGTH * (i // 2) + SQUARE_WIDTH - 1, HEIGHT + 1, SQUARE_WIDTH + COMBINED_LENGTH * (j // 2),"water")
...
class Environment(object):
    ...
    def __init__(self, actr = False):
        ...
        self.objects = None # List of chunks
...
##            reactor.listenTCP(5555, self.actr)
    ...
    def self_adjust(self):
        ...
        print self.main_line, self.x, self.z, self.yaw_aim
    ...
    def mission_running(self):
        ...
        if self.world_state.number_of_observations_since_last_state > 0:
            ...
            self.block_data = observations[u'floor']
        ...
        # Continually adjust direction while moving, jump if meeting obstacle
        if (self.agent_state == self.AGENT_MOVING):
            self.self_adjust()
            if stuff_in_front(self.yaw_aim, self.block_data):
